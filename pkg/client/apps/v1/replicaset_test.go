...
func TestAppsV1_ReplicaSet(t *testing.T) {
	...
	errorClient := mock.NewFakeClient().PrependReactor("get", "replicasets", true, nil, assert.AnError)
	...
	tests := []struct {
		...
		},
		{
			...
			args:   args{resource: "test"},
			want:   result.NewFromError(NewResourceLoadError("ReplicaSet", assert.AnError)),
		},
	}
...
func TestAppsV1_ReplicaSetList(t *testing.T) {
	...
	errorClient := mock.NewFakeClient().PrependReactor("list", "replicasets", true, nil, assert.AnError)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := tt.appsv1.ReplicaSetList()
			if (err != nil) != tt.wantErr {
				t.Errorf("AppsV1.ReplicaSetList() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if !reflect.DeepEqual(got, tt.want) {
				t.Errorf("AppsV1.ReplicaSetList() = %v, want %v", got, tt.want)
			}
		})
	}
}
