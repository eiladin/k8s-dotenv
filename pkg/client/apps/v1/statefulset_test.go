...
func TestStatefulSet(t *testing.T) {
	...
	validate := func(t *testing.T, tc *testCase) {
		t.Run(tc.Name, func(t *testing.T) {
			actualResult := tc.AppsV1.StatefulSet(tc.Resource)
	...
	kubeClient.PrependReactor("get", "statefulsets", true, nil, assert.AnError)

	validate(t, &testCase{
		Name:           "Should return API errors",
		AppsV1:         NewAppsV1(kubeClient, &clientoptions.Clientoptions{Namespace: "test"}),
		Resource:       "test",
		ExpectedResult: result.NewFromError(NewResourceLoadError("DaemonSet", assert.AnError)),
	})
}
...
func TestStatefulSetList(t *testing.T) {
	...
	validate := func(t *testing.T, tc *testCase) {
		t.Run(tc.Name, func(t *testing.T) {
			actualSlice, actualError := tc.AppsV1.StatefulSetList()
	...
	kubeClient.PrependReactor("list", "statefulsets", true, nil, assert.AnError)
